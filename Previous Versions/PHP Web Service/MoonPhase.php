...
class MoonPhase
{
	...
	* @return float
	*/
	protected function kepler($m, $ecc)
	{
		// 1E-6
		$epsilon = 0.000001;
		$e = $m = deg2rad($m);
	...
	* @return float
	*/
	protected function meanphase($date, $k)
	{
	...
	* @return float|null
	*/
	protected function truephase($k, $phase)
	{
		$apcor = false;
	...
	* @return float
	*/
	protected function utc_to_julian($timestamp)
	{
	...
	*/
	...
	{
		return $this->{$property_name} ? $this->{$property_name} : null;
	}
	...
	* @return float
	*/
	public function get_phase($name)
	{
		$phases = [
			'new_moon',
			'first_quarter',
		...
		return $this->quarters[array_flip($phases)[$name]] ? $this->quarters[array_flip($phases)[$name]] : null;
	}
