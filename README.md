...
The [Climacell MicroWeather API](https://www.climacell.co/weather-api/) is a paid service but there is a free option for developers that has limited capabilities and usage limits.  A free account gets 1000 API calls per day.  This script makes four API calls every time it runs.  That means the script could be run 250 times per day or once every 5 minutes 46 seconds.  My dashboard has been doing one call to the DarkSky API every 30 minutes.  That would translate into 192 Climacell API calls per day.
